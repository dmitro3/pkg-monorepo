...
import { Slots_Unity_Methods, PrincessWinr_Unity_Methods } from '../../core/types';
import { usePrincessWinrGameStore } from '../store';

interface UseUnityPrincessWinrParams {
  buildedGameUrl: string;
  buildedGameUrlMobile: string;
}

const log = debug('worker:UsePrincessWinr');

export const useUnityPrincessWinr = ({
  buildedGameUrl,
  buildedGameUrlMobile,
}: UseUnityPrincessWinrParams) => {
  const { gameUrl, setGameUrl, prevWidth, setPrevWidth } = usePrincessWinrGameStore();
  ...
  const {
    ...
    loaderUrl: `${buildedGameUrl}/PrincessWinr.loader.js`,
  ...
  const handleUpdateMultiplier = React.useCallback(
    (multiplier: string) => {
      ...
      sendMessage(
        'WebGLHandler',
        'ReceiveMessage',
        `${PrincessWinr_Unity_Methods.SET_MULTIPLIER}|${multiplier}`
      );
    },
    [sendMessage]
  );
