...
export const MineCellBg = (
  props: SVGProps<SVGSVGElement> & {
    isLoading?: boolean;
  }
) => {
  const { cellImage } = useMinesTheme();

  if (cellImage) {
    return (
      <div className="wr-relative wr-w-full wr-h-full">
        <svg
        ...
        <div className="wr-absolute wr-inset-0 wr-w-full wr-h-full wr-flex wr-items-center wr-justify-center">
          <img
            src={cellImage}
            alt="cell"
            className={cn({
              'wr-animate-mines-pulse': props.isLoading,
            })}
          />
        </div>
      </div>
